...
ptyProcess.onExit(({ exitCode, signal }) => {
  console.log(`PTY process excited with code : ${exitCode}, signal: ${signal}`);
});
...
app.patch("/update-files", async (req, res) => {
  ...
  if (!updates || !Array.isArray(updates)) {
    return res.status(400).json({
      message:
        'Invalid request body. Expected JSON object with an "updates" property containing an array of file updates.',
      status: "error",
    });
  }
...
app.post("/create-files", async (req, res) => {
  ...
  if (!files || !Array.isArray(files)) {
    return res.status(400).json({
      message:
        'Invalid request body. Expected JSON object with an "files" property containing an array of file updates.',
      status: "error",
    });
  }
